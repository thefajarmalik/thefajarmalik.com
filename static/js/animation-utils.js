...
function initSkillsHoverAnimations (sectionSelector) {
  ...
  if (sections.length > 1) {
    console.warn(
      `Multiple sections found with selector: ${sectionSelector}. Using the first one.`
    )
  }
  ...
  cards.forEach((card, index) => {
    ...
    if (!animationContainer) {
      ...
      animationContainer.className =
        'domain-animation absolute inset-0 pointer-events-none overflow-hidden rounded-xl'
    ...
    card.addEventListener('mouseenter', () => {
      ...
      animationElements = createDynamicAnimation(
        animationContainer,
        animationOptions
      )
    })
