...
function initParticleSystem () {
  ...
  if (!particleContainer) {
    ...
    particleContainer.className =
      'particle-container absolute inset-0 pointer-events-none overflow-hidden'
...
function createAndAnimateMorphingShapes (shapesContainer) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    console.log(
      `Created diagonal line ${i + 1} at ${randomX.toFixed(
        1
      )}%, ${randomY.toFixed(1)}% with angle ${randomAngle.toFixed(1)}°`
    )
  }
  ...
  for (let i = 0; i < 3; i++) {
    ...
    console.log(
      `Created spline curve ${i + 1} at ${randomX.toFixed(
        1
      )}%, ${randomY.toFixed(1)}%`
    )
  }
...
function animateDiagonalLines (lines) {
  lines.forEach((line, index) => {
    ...
    tl.to(line, {
      ...
      {
        rotation:
          line.style.transform.replace('rotate(', '').replace('deg)', '') + 180,
        duration: 6,
        ease: 'none'
      },
      0
...
function initMouseTracking () {
  ...
  heroSection.addEventListener('mouseleave', () => {
    ...
    const elementsToReset = [
      ...
      ...diagonalLines,
      ...splineCurves
    ].filter(el => el && !el.hasAttribute('data-natural-animation'))
  ...
  function updateMouseEffects () {
    ...
    morphingShapes.forEach((shape, index) => {
      ...
      const shapeScale =
        1 + Math.abs(normalizedX + normalizedY) * (0.2 + index * 0.1)
    ...
    diagonalLines.forEach((line, index) => {
      ...
      const lineScale =
        1 + Math.abs(normalizedX + normalizedY) * (0.1 + index * 0.05)
    ...
    splineCurves.forEach((curve, index) => {
      ...
      const curveScale =
        1 + Math.abs(normalizedX + normalizedY) * (0.05 + index * 0.02)
    ...
    gsap.to(heroSection, {
      boxShadow: `0 0 ${20 + glowIntensity * 50}px rgba(59, 130, 246, ${
